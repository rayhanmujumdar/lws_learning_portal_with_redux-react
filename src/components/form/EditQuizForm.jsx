...
export default function EditQuizForm({ quizId, control }) {
  const { data: quiz, isSuccess: quizSuccess } = useGetQuizQuery(quizId);
  ...
  useEffect(() => {
    if (quizSuccess) {
      const { question, options, video_id } = quiz;
            ...
            <div className="col-span-6">
              ...
              >
                ...
                {videos?.map((video) => {
                  if (videoId === video.id) {
                    return (
                      <option selected key={video?.id} value={video?.id}>
                        {video?.title}
                      </option>
                    );
                  } else {
